...
def load_data(data_path):
    ...
    with open('D:/20news-bydate/word_idfs.txt', encoding='unicode_escape') as f:
...
def clustering_with_KMeans():
    data, labels = load_data(data_path='D:/20news-bydate/tf_idf_train')
    X = csr_matrix(data)
    print('='*20)
    kmeans = KMeans(
        n_clusters=20,
...
def classifying_with_linear_SVMs():
    train_X, train_y = load_data('D:/20news-bydate/tf_idf_train')
    ...
    test_X, test_y = load_data('D:/20news-bydate/tf_idf_test')
...
def classifying_with_kernel_SVMs():
    ...
    predicted_y = classifier.predict(test_X)
    accuracy = compute_accuracy(predicted_y = predicted_y, expected_y = test_y)
    with open('D:/20news-bydate/session2_result.txt', 'a') as f:
        f.write(f'SVM:\n')
        f.write(f'Accuracy: {accuracy}\n')
